...
class BSJNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new BSJNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawBSJNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : BSJNode {
        var curr : BSJNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
